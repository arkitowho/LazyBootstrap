...
运行中的 `launcher/MediaUpdater.exe` 不直接替换，也不被父目录删除或镜像清理移除。复制此文件时最后写入 `.pending`，下次启动主程序时直接覆盖正式更新器，不创建 `.bak`；占用时重试，失败保留 `.pending`。禁止清单直接删除或编辑更新器，也禁止直接操作 `.pending`、`tmp`、`.media-update`、`update_log.txt` 和 `.media-update-*` 内部路径。
...
1. **启动器解压**：清理 `.media-update/tmp/` 后解压选中的包。
...
```text
.media-update/
├── tmp/                # 解压内容；失败保留，成功尽力清理
└── update_log.txt      # 本次启动器校验与更新器安装共同写入的日志
```
...
XML 预演在读取、解析、每条编辑及序列化边界检查取消。正在进行的单次同步操作结束后即停止，不继续执行剩余编辑，也不将取消包装成 XML 格式错误。

更新后从更新器自身所在目录推导外层 Launcher 的位置，优先 `启动.exe`，其次 `Launcher.exe`，并通过绝对 `--basedir` 传递本次游戏目录。即使程序目录与游戏目录分离，也会重新启动同一套程序；缺少外层入口时不回退到主程序。

更新器使用居中更新卡片，原地显示阶段、当前操作及文件、安装完成项数、进度条和耗时，不滚动输出逐文件历史。常规卡片最多 72 列、15 行，周围留白；成功后切换为独立结果页，隐藏阶段、文件及进度条。游戏目录和日志位置仅在失败、取消或警告时展示，异常卡片最多 22 行。等待和预演不显示百分比；安装进度按最终变更项计数，包括删除、目录创建和文件写入。界面独立刷新，不增加操作延迟；窄窗口自动使用无边框紧凑布局。

失败详情可用上下键或 PageUp/PageDown 滚动，按回车关闭。成功后显示 5 秒重启倒计时；若启动入口缺失或启动失败，保留成功结果和手动启动提示，等待回车。输出重定向或终端绘制不可用时使用纯文本；输入重定向时不等待按键。完整操作记录仍保存在上述日志文件中。

`update_log.txt` 仅记录内部步骤、操作参数、完成数量及错误诊断。内部步骤使用英文；系统、JSON/XML 解析器等原始错误详情和中文路径保持原样。校验器、安装引擎及运行器分别记录自己负责的阶段，同一失败或取消只记录一次；每项安装只在执行前记录一次，随后通过阶段结果说明完成数量。界面标题、中文进度提示、成功标语、倒计时和按键说明不写入日志。校验阶段新建日志，更新器继续追加。

卡片标题为 `Media Updater`；等待动画在“处理中”后循环显示一至五个英文句点，间隔 50 毫秒。错误快照分别提供完整消息、具体原因和目标路径，界面不会通过解析中文标题来去重。

控制台回归：设置 `UPDATE_TEST_UPDATER` 为发布后的 `MediaUpdater.exe` 绝对路径，运行 `python -B -m unittest discover -s Tests -p test_update_console.py -v`。测试在隐藏的独立 Windows 控制台和临时游戏目录中验证真实缓冲区颜色、窗口缩放、错误滚动、按键退出和光标恢复，不接触实际游戏文件。
...
`AppConfigDocument` 负责内存中的 TOML 解析、保留注释的编辑、默认项补齐及预设规范化。Launcher 从同一份文档准备配置；主程序的 `AppConfigStore` 负责实际读取和保存，每次操作重新读取文件。配置快照由配置存储恢复，保存与恢复均只能替换已有且有效的文件。

Launcher 只链接配置文档及默认项等共享源文件，不链接主程序配置存储和日志库。共享文件调整后需要同时验证 Launcher、主程序、更新器及回归项目的编译引用，保持 Tomlyn 源生成和 NativeAOT 支持。

完整验证顺序：Release 构建及 C# 回归 → 三个程序和回归项目的 win-x64 NativeAOT 发布 → Python 校验、示例包及控制台回归 → `Tests/Test-ConfigStartup.ps1`。该启动测试接受 `-PublishRoot`，目录内应包含 `outer/Launcher.exe`、`launcher/` 主程序完整发布内容和 `regression/RegressionTests.exe`；全部测试使用隔离的临时目录。
